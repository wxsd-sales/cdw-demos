/********************************************************
 * Vidcast playlist macro for Cisco RoomOS (Board Pro, etc.)
 *
 * Opens a panel that lists all videos from a public Vidcast playlist
 * and plays the selected item in a WebView on the main display.
 *
 * Pattern follows iptv-macro.js (UI Extensions, WebView, volume, close).
 * Reference only: iptv-macro.js, vidcast-lib.py — do not modify those files.
 *
...
const config = {
   ...
   * - "dataurl": Minimal HTML5 <video> in a data: URL using camera_asset_url (no hosted player).
   * - "share":   https://app.vidcast.io/share/{shareId} (Vidcast-hosted UI; use if data: is blocked).
   * - "direct":  Open the signed MP4 URL directly in WebView (works on some WebEngines).
   */
  playbackMode: "dataurl",
